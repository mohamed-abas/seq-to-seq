...
class deep_s2s_model(base_model):
    def __init__(self,
                 exp_title = '',
    ...
    def build(self):
        ...
        en_x = en_x + self.enc_ad_input
        if self.recurrent_layer == 'Deep_LSTM':
            # LSTM layer_1
            encoder_1 = LSTM(self.num_hidden_cells, return_state=True, return_sequences=True, dropout=self.dropout)
            encoder_outputs_1, state_h1, state_c1 = encoder_1(en_x)
            # LSTM layer_2
        ...
        elif self.recurrent_layer == 'Deep_BI_LSTM':
            # layer_1
            ...
            encoder_outputs, f_state_h, f_state_c, b_state_h, b_state_c = encoder(encoder_outputs_1)
            # concatenation
            state_h = Concatenate()([f_state_h, b_state_h])
            state_c = Concatenate()([f_state_c, b_state_c])
            self.encoder_states = [state_h, state_c]
        elif self.recurrent_layer == 'Deep_GRU':
            # GRU layer_1
            encoder_1 = GRU(self.num_hidden_cells, return_state=True, return_sequences=True, dropout=self.dropout)
            encoder_outputs_1, self.encoder_states_1 = encoder_1(en_x)
            # GRU layer_2
            encoder = GRU(self.num_hidden_cells, return_state=True, return_sequences=True, dropout=self.dropout)
            encoder_outputs, self.encoder_states = encoder(encoder_outputs_1)
        elif self.recurrent_layer == 'Deep_BI_GRU':
            # GRU layer_1
            ...
            encoder_outputs, f_encoder_states, b_encoder_states = encoder(encoder_outputs_1)
    ...
    def train(self,encoder_input_data, decoder_input_data, enc_antigenic_distance,decoder_target_data,
        ...
        callbacks_list = [early_stopping, checkpoint, lr_sched, tensorboard_callback]
